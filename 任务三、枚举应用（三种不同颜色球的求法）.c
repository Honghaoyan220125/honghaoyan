...
int main()
{
	...
	int n,loop;                                                      
	n=0;
	for(i=red;i<=black;i++)                                          //外循环使i的值从red 变black 
	  for(j=red;j<=black;j++)                                        //中循环使j 的值从red变black 
	     if(i!=j)                                                     //如果两球不同色 
	     {
	     	for(k=red;k<=black;k++)                                   // 内循环使k 的值从red变black
	     	if((k!=i)&&(k!=j))                                         //如果三球不同色 
	     	{
	     		n=n+1;                                                //符合条件的次数加一 
	     		printf("%-4d",n);                                   //输出当前使第几个符合条件的组合 
